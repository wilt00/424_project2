...
worstCountiesMap <- function(mapType, selectedYear, numDisplayed) {
  ...
  maxVals <- datasource[
    order(datasource[[mapDataCol]],decreasing=TRUE)[0:n],
    ]

  # Use inner_join from dplyr so that order of rows is preserved
  maxValsCounties <- inner_join(counties, maxVals, by=c("region","subregion"))

  ggplot(data=map_data_states, mapping=aes(x=long, y=lat, group=group)) +
    geom_polygon(color="black", fill="gray") +
    geom_polygon(data=maxValsCounties, aes_string(fill=mapDataCol), color="white") +
    coord_fixed(1.3) +
    guides(fill=FALSE) +
    theme_nothing()
}

countyHeatmap <- function(mapType, selectedYear, selectedMonth, selectedDay) {
  datasource <-
