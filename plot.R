...
sites$StateCounty <-
  with(sites, paste0(State.Name, "#", County.Name))
...
AqiLineColumns <-
  c("Year", "Max.AQI", "X90th.Percentile.AQI", "Median.AQI")
...
PollutantColumnNames <-
  c("Days.CO",
    ...
    "Days.PM2.5",
    "Days.PM10")
PollutantFactorNames <-
  c("CO", "NO2", "Ozone", "SO2", "PM2.5", "PM10", "Other")
pollutantPalette <- c(
  "CO" = "#E69F00",
  "NO2" = "#56B4E9",
  "Ozone" = "#F0E442",
...
aqi_pie <- function(selectedYear,
  ...
  aabc_yr <-
    subset(
      aabc,
      Year == selectedYear &
        tolower(State) == tolower(selectedState) &
        tolower(County) == tolower(selectedCounty)
    )
  ...
  aqi_mx_t_df <-
    cbind(DayKind = factor(rownames(aqi_mx_t_df), levels = AqiColumnNames), aqi_mx_t_df)

  # Create percentage column
  aqi_mx_t_df$Pct <-
    with(aqi_mx_t_df, round((Days / daysWithAqi) * 100))
  ...
  ggplot(aqi_mx_t_df, aes(
    ...
    geom_label(data = subset(aqi_mx_t_df, Pct > 0),
               aes(
                 label = paste0(Pct, "%"),
                 x = 1.8,
                 y = RealMidpt
...
aqi_table <- function(selectedYear,
                      selectedState,
                      selectedCounty) {
...
pollutant_pie <-
  function(selectedYear,
    ...
    df_label_positions <-
      c(round(daysWithAqi * .75), round(daysWithAqi * .25))
    ...
    pollutant_df$pct <-
      with(pollutant_df, round((df_days / daysWithAqi) * 100))

    # Mucking around with factors so the colors come out right in ggplot
    pollutant_df$df_types <-
      with(pollutant_df, factor(
        df_labels,
        levels = rev(PollutantFactorNames),
        ordered = TRUE
      ))

    ggplot(pollutant_df, aes(
      x = "",
      y = df_days,
      fill = df_types,
      label = pct
...
pollutant_bar <-
  function(selectedYear,
    ...
    # Convert back into dataframe and rename data column for reference
    all_pollutants_df <-
      data.frame(t(as.matrix(aabc_yr[PollutantColumnNames])), stringsAsFactors =
                   FALSE)
    colnames(all_pollutants_df) <- c("Days")
    all_pollutants_df$PollutantRowNames <-
      rownames(all_pollutants_df)
    all_pollutants_df$Pollutants <-
      with(all_pollutants_df, sub('.....', '', PollutantRowNames))

    ggplot(all_pollutants_df,
           aes(
             x = Pollutants,
             y = Days,
             fill = Pollutants,
...
pollutant_line <- function(selectedState, selectedCounty) {
  pcColNames <-
    append(PollutantColumnNames, c("Year", "Days.with.AQI"))
  aabc_region <-
    subset(
      aabc,
      tolower(State) == tolower(selectedState) &
  ...
  aabc_region$PctCO <-
    with(aabc_region, (Days.CO / Days.with.AQI) * 100)
  aabc_region$PctNO2 <-
    with(aabc_region, (Days.NO2 / Days.with.AQI) * 100)
  aabc_region$PctOzone <-
    with(aabc_region, (Days.Ozone / Days.with.AQI) * 100)
  aabc_region$PctSO2 <-
    with(aabc_region, (Days.SO2 / Days.with.AQI) * 100)
  aabc_region$PctPM2.5 <-
    with(aabc_region, (Days.PM2.5 / Days.with.AQI) * 100)
  aabc_region$PctPM10 <-
    with(aabc_region, (Days.PM10 / Days.with.AQI) * 100)

  ggplot(aabc_region) +
    geom_line(aes(x = Year, y = PctCO, group=1, color = "CO")) +
    geom_line(aes(x = Year, y = PctNO2, group=1, color = "NO2")) +
    geom_line(aes(x = Year, y = PctOzone, group=1,  color = "Ozone")) +
    geom_line(aes(x = Year, y = PctSO2, group=1, color = "SO2")) +
    geom_line(aes(x = Year, y = PctPM2.5, group=1, color = "PM2.5")) +
    geom_line(aes(x = Year, y = PctPM10, group=1, color = "PM10")) +
...
pollutant_table <- function(selectedState, selectedCounty) {
  ...
  aabc_region$Percent.CO <-
    with(aabc_region, round((Days.CO / Days.with.AQI) * 100), digits = 1)
  aabc_region$Percent.NO2 <-
    with(aabc_region, round((Days.NO2 / Days.with.AQI) * 100), digits = 1)
  aabc_region$Percent.Ozone <-
    with(aabc_region, round((Days.Ozone / Days.with.AQI) * 100), digits = 1)
  aabc_region$Percent.SO2 <-
    with(aabc_region, round((Days.SO2 / Days.with.AQI) * 100), digits = 1)
  aabc_region$Percent.PM2.5 <-
    with(aabc_region, round((Days.PM2.5 / Days.with.AQI) * 100), digits = 1)
  aabc_region$Percent.PM10 <-
    with(aabc_region, round((Days.PM10 / Days.with.AQI) * 100), digits = 1)

  return(aabc_region[c(
    "Year",
    "Percent.CO",
    "Percent.NO2",
    "Percent.Ozone",
...
mapCounty <- function(selectedState, selectedCounty) {
  loc <-
    subset(locations,
           tolower(StateCounty) == paste0(tolower(selectedState), "#", tolower(selectedCounty)))
  if (nrow(loc) < 1) {
  ...
  map <- leaflet() %>%
    ...
    addMarkers(
      lng = n,
      lat = t,
      popup = paste0(selectedCounty, " county, ", selectedState)
    ) %>%
